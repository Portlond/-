...
{
    ui->setupUi(this);
    window1=new save_buy;
    window2=new redak_buy;
    ...
    connect(window2,&redak_buy::obnov_redak,this,&buy::obnov);
}
...
void buy::on_sprav_clicked()
{
    window = new SPRPOLZOVAT(this);
    window -> show();
}
void buy::on_udalen_clicked()
{
...
void buy::on_dopol_clicked()
{
    window1 ->show();
}
void buy::on_tableView_clicked(const QModelIndex &index)
{
    row = index.row();
}

void buy::on_vihod_clicked()
{
...
void buy::on_obnov_clicked()
{
    QString id;
    QSqlQuery qry;
    qry.prepare("select * from Закупка");
    model = new QSqlTableModel(this,db);
    model->setTable("Закупка");
    model->select();
    ui->tableView->setModel(model);
...
void buy::on_poisk_clicked()
{

char percent = '%';
QSqlQuery query;

QString querySearchString = QString("ID_закупки LIKE '%1%2' OR Дата LIKE '%1%2' OR Стоимость LIKE '%1%2' OR Название_товара LIKE '%1%2' OR Количество LIKE '%1%2'")
